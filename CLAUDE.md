...
**Smart routing (2.13)**: `services/route_classifier.py` (puro, señales → score → tier `trivial|simple|standard|complex`), `services/smart_router.py` (`decide()` envuelve `pick_provider`; reglas explícitas primero, luego tabla `smart.tiers`, filtros por capacidad/salud/presupuesto/superficie; `mode=shadow` solo registra, `mode=active` decide), `services/health_service.py` (estado por destino `provider:<id>` / `cli:<id>` persistido en `routing_state.json`), `services/budget_service.py`, `services/decisions_log.py` (tabla `route_decisions` en usage.db), `core/quota_signals.py` (regex de cuota/auth/sobrecarga compartidas). `/v1/*` devuelve `X-Bipolar-Route` y `X-Bipolar-Decision-Id`; acepta `X-Bipolar-Tier`. Con `smart.enabled=False` el comportamiento es el previo.

**Delegación a agentes CLI (2.13)**: `services/cli_agents/registry.py` (detección, `--version`, `codex login status`, `agy -p "/usage"` y `/model` sin gastar cuota, `/api/tags` de Ollama), `services/cli_agents/adapters.py` (argv en lista, tarea por stdin o archivo puntero `.bipolar/jobs/<id>/task.md`, env mínimo sin secretos ni `ANTHROPIC_BASE_URL`, flags de seguridad fijos por CLI), `services/cli_agents/broker.py` (workspace allow-list cerrada por defecto, clasificación, elección por tier/cuota/concurrencia, reintento solo ante señal de cuota, SSE). Endpoints en `api/smart.py` (`/api/smart/*`) y `api/delegate.py` (`/api/delegate/jobs*`). Modelos en `models/smart.py` y `models/delegate.py`; el registro siembra `cli_agents` deshabilitados y una tabla de tiers inicial (`_seed_smart_defaults`). Spec: `docs/superpowers/specs/2026-09-10-gestion-inteligente-routing-y-delegacion-design.md`.
